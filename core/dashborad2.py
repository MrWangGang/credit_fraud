...
with bot_left:
    fig_scatter = px.scatter(
        df.sample(1500), x='amt', y='city_pop', color='is_fraud',
...
fraud_by_amt_range['fraud_prob'] = fraud_by_amt_range['sum'] / fraud_by_amt_range['count']
...
with left_col:
    ...
    bins = [i for i in range(min(fraud_age), max(fraud_age), (max(fraud_age) - min(fraud_age)) // 20)]
